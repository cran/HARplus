...
# Saving Data to GEMPACK `.HAR` Format

This function is particularly useful when:
- Creating custom HAR files for simulation inputs or aggregated datasets  
- Converting R-based results into GEMPACK-readable format  
- Exporting modified or merged results from GTAP-style models for use in RunDynam or RunGTAP
...
**Note:**  
To rename dimensions, first rename the corresponding column names in your data (e.g., using `dplyr::rename()` or `data.table::setnames()`).  
Each dimension name in a `.HAR` file must be no longer than **four characters**.

```{r save_har_single, eval=FALSE}
